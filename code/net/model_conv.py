...
class conv_2(nn.Module):
    def __init__(self, in_channels, out_channels, k_size, stride, dropout=0.1):
        super().__init__()
...
class conv_2_3(nn.Module):
    def __init__(self, in_channels, out_channels,dropout=0.1):
        super().__init__()
        self.cn3 = nn.Sequential(nn.BatchNorm2d(in_channels),
                                 nn.Sigmoid(),
                                 nn.Conv2d(in_channels, out_channels, (2, 3), 2),
                                 nn.BatchNorm2d(out_channels),
                                 nn.Dropout(dropout, inplace=True))
        self.sigm = nn.Sigmoid()
...
class conv_3_3(nn.Module):
    def __init__(self, in_channels, out_channels, dropout=0.1):
        super().__init__()
        self.cn3_3 = nn.Sequential(nn.BatchNorm2d(in_channels),
                                 nn.Sigmoid(),
...
class conv_5_3(nn.Module):
    def __init__(self, in_channels, out_channels, dropout=0.1):
        super().__init__()
        self.cn5_3 = nn.Sequential(nn.BatchNorm2d(in_channels),
                                 nn.Sigmoid(),
                                 nn.Conv2d(in_channels, out_channels, (5, 3), 2),
                                 nn.BatchNorm2d(out_channels),
                                 nn.Dropout(dropout, inplace=True))
        self.sigm = nn.Sigmoid()
...
class node2_conv(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv3 = conv_2_3(in_channels, out_channels)

        self.conv2_1 = conv_2(in_channels, 2, (1, 2), 1)
        self.conv2_2 = conv_2(2, out_channels, 2, 2)
    ...
    def forward(self, x):
        ...
        x = x[:,:,0,:].reshape(x.shape[0], x.shape[1], 1, x.shape[3]) + x2 + x3
...
class node3_conv(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv3  =  conv_3_3(in_channels, out_channels)

        self.conv2_1 = conv_2(in_channels, 2, 2, 1)
        self.conv2_2 = conv_2(2, out_channels, 2, 2)
...
class node5_conv(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv3 = conv_5_3(in_channels, out_channels)

        self.conv2_1 = conv_2(in_channels, 2, 2, 1)
        self.conv2_2 = conv_2(2, 2, 2, 1)
        self.conv2_3 = conv_2(2, out_channels, (2, 1), 2)
...
class node25_conv(nn.Module):
    def __init__(self, in_channels, out_channels, dropout=0.1):
        super().__init__()
        self.conv25 = nn.Sequential(nn.BatchNorm2d(in_channels),
                                 nn.Tanh(),
                                 nn.Conv2d(in_channels, out_channels, (25, 1), 1),
                                 nn.BatchNorm2d(out_channels),
                                 nn.Dropout(dropout, inplace=True))
        self.Tanh = nn.Tanh()
